...
const SplitSize = () => {
  ...
  const handleSplit = async () => {
    if (files.length === 0 || !maxSize) return;
      ...
      setProgress(prev => {
        if (prev >= 100) {
          ...
          const splitFiles = files.flatMap(file => [
            { name: `${file.name.replace('.pdf', '')}_part1.pdf`, url: '#' },
            { name: `${file.name.replace('.pdf', '')}_part2.pdf`, url: '#' },
            { name: `${file.name.replace('.pdf', '')}_part3.pdf`, url: '#' }
          ]);
        ...
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-8">
            <div className="flex items-center justify-center gap-3 mb-4">
              ...
              <h1 className="text-4xl font-bold">Split by Size</h1>
            </div>
            <p className="text-muted-foreground text-lg">
              Create smaller files with specific sizes
            </p>
          </div>
            ...
            <Card className="mb-8">
              ...
              <CardContent>
                <div className="space-y-4">
                  <div>
                    <Label htmlFor="maxSize">Maximum file size per part</Label>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      Each resulting file will be smaller than this size.
                    </p>
                  </div>
                </div>
              </CardContent>
            ...
            <Card className="mb-8">
              ...
              <CardContent>
                ...
                <div className="mt-6">
                  <Button 
                    onClick={handleSplit} 
                    disabled={isProcessing || !maxSize}
                    className="w-full"
                    size="lg"
                  >
                    <Split className="h-4 w-4 mr-2" />
                    {isProcessing ? "Splitting..." : `Split by ${maxSize} ${sizeUnit}`}
                  </Button>
                </div>
              </CardContent>
            </Card>
          ...
          {isProcessing && (
            <Card className="mb-8">
              <CardHeader>
                <CardTitle>Splitting PDF by size...</CardTitle>
              </CardHeader>
              <CardContent>
